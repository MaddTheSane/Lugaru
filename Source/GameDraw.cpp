...
int Game::DrawGLScene(StereoSide side)
{
    ...
    if (!mainmenu) {
        ...
        if (!cellophane) {
            ...
            glCullFace(GL_FRONT);
            glDepthMask(1);
            for (size_t k = 0; k < Person::players.size(); k++) {
                if (k == 0 || tutoriallevel != 1) {
                    glEnable(GL_BLEND);
                    glEnable(GL_LIGHTING);
        ...
        if (!console) {
            ...
            if ((texttoggle || editorenabled) && debugmode && !mainmenu) {
                ...
                if (editorenabled) {
                    ...
                    sprintf (string, "Player %lu: numwaypoints: %d", Person::players.size()-1, Person::players.back()->numwaypoints);
        ...
        if (!console) {
            ...
            for (int i = 0; i < 15; i++)
                if ((i != 0 || chatting) && displaytime[i] < 4)
                    for (int j = 0; j < displaychars[i]; j++) {
        ...
        if (drawmode != normalmode) {
            ...
            if (drawmode == doublevisionmode) {
                float crosseyedness = abs(Person::players[0]->damage - Person::players[0]->superpermanentdamage - (Person::players[0]->damagetolerance - Person::players[0]->superpermanentdamage) * 1 / 2) / 30;
                if (crosseyedness > 1)
                    crosseyedness = 1;
                if (crosseyedness < 0)
                    crosseyedness = 0;
        ...
        if (console) {
            ...
            for (int i = 0; i < 15; i++)
                for (int j = 0; j < consolechars[i]; j++) {

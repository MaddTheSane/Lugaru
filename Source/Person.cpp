...
bool Person::DoBloodBigWhere(float howmuch,int which, XYZ where) {
	...
	if(bloodtoggle&&decals&&tutoriallevel!=1){
		...
		if(whichtri!=-1){
			...
			s =  dot(&temp,&N)/length(N);
			CrossProduct(p2-p1,p1-p0,&temp);
			t = dot(&temp,&N)/length(N);
			r = 1 - (s + t);*/
...
glRotatef(-skeleton.muscles[j].rotate2+90,0,0,1);
glRotatef(-skeleton.muscles[j].rotate3,0,1,0);
glScalef(1,simd::distance(skeleton.muscles[j].parent1->position,skeleton.muscles[j].parent2->position),1);
glTranslatef(collisionpoint.x,collisionpoint.y,collisionpoint.z);
glGetFloatv(GL_MODELVIEW_MATRIX,M);
collisionpoint.x=M[12];
collisionpoint.y=M[13];
	...
	if(!skeleton.free){
		...
		if(targetanimation==jumpupanim||targetanimation==jumpdownanim||isFlip()){
			...
			if(id==0){
				...
				OPENAL_SetVolume(channels[whooshsound], 64*length(velocity)/5);
			}
							...
							if((animation[targetanimation].attack==normalattack||targetanimation==walljumprightkickanim||targetanimation==walljumpleftkickanim)&&(!feint)&&(victim->skeleton.free!=2||targetanimation==killanim||targetanimation==dropkickanim||targetanimation==crouchstabanim||targetanimation==swordgroundstabanim||targetanimation==staffgroundsmashanim)){
								...
								if(targetanimation==knifethrowanim&&animation[targetanimation].label[currentframe]==5){
									if(weaponactive!=-1){
										...
										XYZ aim;
										weapons.owner[weaponids[0]]=-1;
										aim=victim->coords+DoRotation(victim->skeleton.joints[victim->skeleton.jointlabels[abdomen]].position,0,victim->rotation,0)*victim->scale+victim->velocity*distance(victim->coords,coords)/50-(coords+DoRotation(skeleton.joints[skeleton.jointlabels[righthand]].position,0,rotation,0)*scale);
										Normalise(aim);
...
void Person::DoStuff(){
	...
	if(onfire){
		...
		deathbleeding=1;
		if(burnt>.6)burnt=.6;
		OPENAL_SetVolume(channels[stream_firesound], 256+256*length(velocity)/3);
	...
	if(skeleton.free==1){
		...
		if(length(average)<10&&dead&&skeleton.free){
			skeleton.longdead+=(2000-length(average))*multiplier+multiplier;
		...
		if(!skeleton.freefall&&freefall&&((jumpkeydown&&jumpkeydowntime<.2)||(hasstaff&&rabbitkickragdoll))&&!dead){
			if (velocity.y > -30) {
				...
				skeleton.free=0;
				if(dot(skeleton.forward,tempvelocity)<0){
					targetanimation=rollanim;
					targetframe=2;
				}
				else{
					...
					if(objects.type[i]==bushtype)
						if(findDistancefastflat(&coords,&objects.position[i])<objects.scale[i]*objects.scale[i]*12&&findDistancefast(&coords,&objects.position[i])<objects.scale[i]*objects.scale[i]*49){
							...
							if(objects.messedwith[i]<=0){
								...
								OPENAL_SetVolume(channels[bushrustle], 40*length(velocity));
								OPENAL_SetPaused(channels[bushrustle], false);

								if(id==0){
									envsound[numenvsounds]=coords;
									envsoundvol[numenvsounds]=4*length(velocity);
									envsoundlife[numenvsounds]=.4;
									numenvsounds++;
								}

								int howmany = 0;
								if(environment==grassyenvironment)howmany=length(velocity)*4;
								if(environment==snowyenvironment)howmany=length(velocity)*2;
								if(detail==2)
				...
				*/
				...
				if(!dead&&targetanimation!=hurtidleanim)
					...
					if(targetanimation==jumpupanim||targetanimation==jumpdownanim||isFlip()){
						...
						flatvelocity.y=0;
						flatvelspeed=length(flatvelocity);
...
int Person::DrawSkeleton() {
	...
	if((frustum.SphereInFrustum(coords.x,coords.y+scale*3,coords.z,scale*8)&&findDistancefast(&viewer,&coords)<viewdistance*viewdistance)||skeleton.free==3){
		...
		framemult=.01;
		updatedelaychange=-framemult*4*(45-simd::distance(viewer,coords)*1);
				...
				if(weaponactive==k){
					if(weapons.type[i]==knife){
						...
						if((currentanimation==crouchstabanim&&targetanimation==crouchstabanim)||(currentanimation==backhandspringanim&&targetanimation==backhandspringanim)){
							...
							temppoint1.y=0;
							temppoint2.y=0;
							weapons.rotation1[i]=acos((temppoint1.z-temppoint2.z)/simd::distance(temppoint1,temppoint2));
		...
		for (int j=0;j<model->TriangleNum;j++){
			if(model->facenormals[j].y<=slopethreshold){
				...
				if(distance<radius){
					...
					if(dot(model->facenormals[j],end)>0&&intersecting){
						start=*p1;
						end=*p1;
						end.y-=radius;
				...
				if(distance<radius*.5){
					...
					if(dot(model->facenormals[j],end)>0&&intersecting){
						if((targetanimation==jumpdownanim||targetanimation==jumpupanim||isFlip())){
							start=velocity;
							velocity-=DoRotation(model->facenormals[j],0,*rotate,0)*length(velocity)*abs(normaldotproduct(velocity,DoRotation(model->facenormals[j],0,*rotate,0)));//(distance-radius*.5)/multiplier;

...
{
    const int whichpatchx=p1->x/(terrain.size/subdivision*terrain.scale);
    const int whichpatchz=p1->z/(terrain.size/subdivision*terrain.scale);
                        ...
                        if (occluded[i] < 6) {
                            ...
                            if (type[i] == bushtype) {
                                ...
                                if (rotxvel[i] || rotx[i]) {
                                    ...
                                    if (rotxvel[i] < 0) rotxvel[i] += multiplier * 4;
                                    if (abs(rotx[i]) < multiplier * 4)
                                        rotx[i] = 0;
                                    if (abs(rotxvel[i]) < multiplier * 4)
                                        rotxvel[i] = 0;
                                ...
                                if (rotyvel[i] || roty[i]) {
                                    ...
                                    if (rotyvel[i] < 0) rotyvel[i] += multiplier * 4;
                                    if (abs(roty[i]) < multiplier * 4)
                                        roty[i] = 0;
                                    if (abs(rotyvel[i]) < multiplier * 4)
                                        rotyvel[i] = 0;
...
void Objects::MakeObject(int atype, XYZ where, float ayaw, float ascale)
{
    if ((atype != treeleavestype && atype != bushtype) || foliage == 1) {
        ...
        if (atype == treeleavestype)
            scale[numobjects] += abs((float)(Random() % 100) / 900) * ascale;
...
void Objects::MakeObject(int atype, XYZ where, float ayaw, float apitch, float ascale)
{
    if ((atype != treeleavestype && atype != bushtype) || foliage == 1) {
        ...
        if (friction[numobjects] == 1.5 && abs(apitch) > 5)
        ...
        for (int i = 0; i < numobjects; i++) {
            if (type[i] != treeleavestype && type[i] != treetrunktype && type[i] != bushtype && type[i] != firetype) {
                for (int j = 0; j < model[i].vertexNum; j++) {
                    ...
                    shadowed[i] = 0;
                    int patchx = terrainpoint.x / (terrain.size / subdivision * terrain.scale);
                    int patchz = terrainpoint.z / (terrain.size / subdivision * terrain.scale);

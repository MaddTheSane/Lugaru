...
unsigned int crouchkey = 0, jumpkey = 0, forwardkey = 0, chatkey = 0, backkey = 0, leftkey = 0, rightkey = 0, drawkey = 0, throwkey = 0, attackkey = 0;
unsigned int consolekey = 0;
...
void Game::inputText(char* str, int* charselected, int* nb_chars)
{
    ...
    while (SDL_PollEvent(&evenement)) {
        ...
        case SDL_TEXTINPUT:
        {
            //::fprintf(stderr, "%s", evenement.text.text);
            int sLen = 1;
            //strcat(&str[*charselected], evenement.text.text);
            //int sLen = (int)strlen(evenement.text.text);
            for (int i = 255; i >= (*charselected) + 1; i--) {
                str[i] = str[i - 1];
            }
            str[*charselected] = evenement.text.text[0];
            (*charselected) += sLen;
            (*nb_chars) += sLen;
            if ((*nb_chars) >= 60) {
                continue;
...
void Game::setKeySelected()
{
    ...
    SDL_Thread* thread = ::SDL_CreateThread(setKeySelected_thread, "KeyThread", NULL);
    if ( thread == NULL ) {

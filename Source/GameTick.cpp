...
static void ch_save(const char *args)
{
    ...
    for (int k = 0; k < numdialogues; k++) {
        ...
        for (int l = 0; l < numdialogueboxes[k]; l++) {
            ...
            int templength = (int)strlen(dialoguetext[k][l]);
            ...
            templength = (int)strlen(dialoguename[k][l]);
            fpackf(tfile, "Bi", templength);
            for (int m = 0; m < templength; m++) {
    ...
    for (int k = 0; k < Person::players[0]->numclothes; k++) {
        int templength = (int)strlen(Person::players[0]->clothes[k]);
        fpackf(tfile, "Bi", templength);
    ...
    for (int i = 0; i < numhotspots; i++) {
        ...
        int templength = (int)strlen(hotspottext[i]);
        fpackf(tfile, "Bi", templength);
        for (int l = 0; l < templength; l++)
    ...
    for (int j = 1; j < Person::players.size(); j++) {
        ...
        if (Person::players[j]->numclothes)
            for (int k = 0; k < Person::players[j]->numclothes; k++) {
                int templength;
                templength = (int)strlen(Person::players[j]->clothes[k]);
                fpackf(tfile, "Bi", templength);
                for (int l = 0; l < templength; l++)
...
void Loadlevel(const char *name)
{
    ...
    if (tfile) {
        ...
        if (!stealthloading) {
            funpackf(tfile, "Bf Bf Bf Bf Bf Bi", &tmpx, &tmpy, &tmpz, &Person::players[0]->yaw, &Person::players[0]->targetyaw, &Person::players[0]->num_weapons);
            Person::players[0]->coords = XYZ{tmpx, tmpy, tmpz};
        }
        if (stealthloading) {
        ...
        if (mapvers >= 8) {
            ...
            for (int k = 0; k < numdialogues; k++) {
                ...
                for (int l = 0; l < 10; l++) {
                    ...
                    participantlocation[k][l] = {tmpx, tmpy, tmpz};
        ...
        for (int i = 0; i < objects.numobjects; i++) {
            funpackf(tfile, "Bi Bf Bf Bf Bf Bf Bf", &objects.type[i], &objects.yaw[i], &objects.pitch[i], &tmpx, &tmpy, &tmpz, &objects.scale[i]);
            objects.position[i] = {tmpx, tmpy, tmpz};
            if (objects.type[i] == treeleavestype)
        ...
        if (numplayers > 1) {
            for (int i = 1; i < numplayers; i++) {
                ...
                funpackf(tfile, "Bi Bi Bf Bf Bf Bi", &Person::players[i - howmanyremoved]->whichskin, &Person::players[i - howmanyremoved]->creature, &tmpx, &tmpy, &tmpz, &Person::players[i - howmanyremoved]->num_weapons);
                Person::players[i - howmanyremoved]->coords = {tmpx, tmpy, tmpz};
                if (mapvers >= 5)
                ...
                if (!removeanother) {
                    ...
                    for (int j = 0; j < Person::players[i - howmanyremoved]->numwaypoints; j++) {
                        funpackf(tfile, "Bf Bf Bf", &tmpx, &tmpy, &tmpz);
                        Person::players[i - howmanyremoved]->waypoints[j] = {tmpx, tmpy, tmpz};
                        if (mapvers >= 5)
...
void doAttacks()
{
    ...
    bool playerrealattackkeydown = 0;
                ...
                for (int j = 0; j < numpathpoints; j++)
                    for (int k = 0; k < numpathpointconnect[j]; k++) {
                        ...
                        if (sq(tempdist) < closestdistance)
                            if (simd::distance(colpoint, pathpoint[j]) + simd::distance(colpoint, pathpoint[pathpointconnect[j][k]]) <
                                    simd::distance(pathpoint[j], pathpoint[pathpointconnect[j][k]]) + .1) {
            ...
            if (Person::players[i]->aiupdatedelay < 0 &&
                ...
                if (distsq(&Person::players[0]->coords, &Person::players[i]->coords) <
                    ...
                    targetpoint += Person::players[0]->velocity *
                                   simd::distance(Person::players[0]->coords, Person::players[i]->coords) / simd::length(Person::players[i]->velocity);
...
void Game::LoadMenu()
{
    ...
    case 9:
        for (int i = 0; i < numchallengelevels; i++) {
            ...
            for (int j = (int)strlen(temp); j < 17; j++)
                strcat(temp, " ");
            name += temp;
            sprintf (temp, "%d", (int)accountactive->getHighScore(i));
            for (int j = (int)strlen(temp); j < (32 - 17); j++)
                strcat(temp, " ");
            name += temp;
...
void MenuTick()
{
    ...
    if (Input::isKeyDown(SDLK_q) && Input::isKeyDown(SDLK_LGUI)) {
        tryquit = 1;
        if (mainmenu == 3) {
            SaveSettings();
        }
...
void Game::Tick()
{
    ...
    */
    ...
    if (!mainmenu) {
        ...
        if (console)
            freeze = 1;
        if (console && !Input::isKeyDown(SDLK_LGUI)) {
            ...
            if (indialogue != -1) {
                ...
                if (directing) {
                    ...
                    if (     Input::isKeyDown(SDLK_KP_1) ||
                             Input::isKeyDown(SDLK_KP_2) ||
                             Input::isKeyDown(SDLK_KP_3) ||
                             Input::isKeyDown(SDLK_KP_4) ||
                             Input::isKeyDown(SDLK_KP_5) ||
                             Input::isKeyDown(SDLK_KP_6) ||
                             Input::isKeyDown(SDLK_KP_7) ||
                             Input::isKeyDown(SDLK_KP_8) ||
                             Input::isKeyDown(SDLK_KP_9) ||
                             Input::isKeyDown(SDLK_KP_0)) {
                        int whichend;
                        if (Input::isKeyDown(SDLK_KP_1)) whichend = 1;
                        if (Input::isKeyDown(SDLK_KP_2)) whichend = 2;
                        if (Input::isKeyDown(SDLK_KP_3)) whichend = 3;
                        if (Input::isKeyDown(SDLK_KP_4)) whichend = 4;
                        if (Input::isKeyDown(SDLK_KP_5)) whichend = 5;
                        if (Input::isKeyDown(SDLK_KP_6)) whichend = 6;
                        if (Input::isKeyDown(SDLK_KP_7)) whichend = 7;
                        if (Input::isKeyDown(SDLK_KP_8)) whichend = 8;
                        if (Input::isKeyDown(SDLK_KP_9)) whichend = 9;
                        if (Input::isKeyDown(SDLK_KP_0)) whichend = 0;
                                                        ...
                                                        if (normaldotproduct(Person::players[i]->facing, Person::players[i]->coords - Person::players[j]->coords) < 0 &&
                                                            ...
                                                            if (Person::players[i]->isFlip()) {
                                                                if (Person::players[i]->weaponactive != -1) {
                                                                    ...
                                                                    aim = Person::players[i]->victim->coords + DoRotation(Person::players[i]->victim->jointPos(abdomen), 0, Person::players[i]->victim->yaw, 0) * Person::players[i]->victim->scale + Person::players[i]->victim->velocity * simd::distance(Person::players[i]->victim->coords, Person::players[i]->coords) / 50 - (Person::players[i]->coords + DoRotation(Person::players[i]->jointPos(righthand), 0, Person::players[i]->yaw, 0) * Person::players[i]->scale);
                                                                    Normalise(aim);
    ...
    if (!cameramode) {
        ...
        if (Person::players[0]->skeleton.free != 2/*&&!autocam*/) {
            ...
            if (findLengthfast(&Person::players[0]->velocity) > 400) {
                cameraspeed = 20 + (simd::length(Person::players[0]->velocity) - 20) * .96;
            }
            ...
            colviewer = viewer;
            coltarget = cameraloc;
            objects.SphereCheckPossible(&colviewer, simd::distance(colviewer, coltarget));
            ...
            cameradist = simd::distance(viewer, target);
